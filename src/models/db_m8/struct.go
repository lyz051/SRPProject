...
type ACLine struct {
	...
	Delete       int       `json:"delete"`                         //删除标志位
	Type         string    `json:"type"`                           //卡片类型
	STID_A       string    `gorm:"column:stid_a"`                  //A端厂站
	STID_B       string    `gorm:"column:stid_b"`                  //B端厂站
	Bus_A        string    `gorm:"column:bus_a"`                   //A端母线名
	Bus_B        string    `gorm:"column:bus_b"`                   //B端母线名
	Voltage      float32   `json:"voltage"`                        //电压等级
...
type DCLine struct {
	...
	Delete      int       `json:"delete"`                         //删除标志位
	Type        string    `json:"type"`                           //卡片类型
	ZLBus       string    `gorm:"column:zl_bus"`                  //整流侧母线名
	ZLVolt      float32   `gorm:"column:zl_volt"`                 //整流侧基电压
	NBBus       string    `gorm:"column:nb_bus"`                  //逆变侧母线名
	NBVolt      float32   `gorm:"column:nb_volt"`                 //逆变侧基电压
	ParameterID int       `json:"parameter_id"`                   //当前使用的参数ID
	DCSys       string    `gorm:"column:dc_sys"`                  //所属直流系统
}
...
type Conventor struct {
	...
	Delete  int       `json:"delete"`                         //删除标志位
	Type    string    `json:"type"`                           //卡片类型
	Bus1    string    `gorm:"column:bus_1"`                   //换流器母线名
	BusV1   float32   `gorm:"column:bus_v_1"`                 //换流器母线电压
	Bus2    string    `gorm:"column:bus_2"`                   //换流变一次侧母线名
	BusV2   float32   `gorm:"column:bus_v_2"`                 //换流变一次侧基电压
